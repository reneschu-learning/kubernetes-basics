...
Let's apply a Taint to the `desktop-worker2` node to repel all Pods except those that have a matching Toleration. You can do this using the following command:

```bash
kubectl taint nodes desktop-worker2 workloads=frontend:NoSchedule
```

Watch the Pods from the `gallery-dep` Deployment in `k9s`. So far, nothing happend because the Pods are already running on the `desktop-worker2` node and the Taint effect only applies to new Pods (`NoSchedule`). However, when you try to scale the Deployment (e.g., `kubectl scale deployment gallery-dep --replicas=5`), you will see that the new Pods are stuck in a pending state because they cannot be scheduled on the `desktop-worker2` node due to the Taint. In addition, Kubernetes cannot schedule them on another node because the `nodeSelector`. You can see the error when describing the Pods in `k9s` (`d` key).
...
Watch the Pods from the `gallery-dep` Deployment in `k9s`. The updated manifest has recreated the Pods and they were scheduled on the `desktop-worker2` node because they now have a matching Toleration for the Taint applied to that node. You can also scale the Deployment again (e.g., `kubectl scale deployment gallery-dep --replicas=5`) and see that the new Pods are successfully scheduled on the `desktop-worker2` node.
...
And Tolerations must exactly match the Taint in order for the Pod to be scheduled on the node. This includes the key, value, and effect of the Taint. Let's create another problem by changing the Taint on the `desktop-worker2` node to have a different value. You can do this using the following command:

```bash
# Remove the old Taint and add a new one with a different value
kubectl taint nodes desktop-worker2 workloads=frontend:NoSchedule-
kubectl taint nodes desktop-worker2 workloads=frontend:NoExecute
```

As soon as you apply the new Taint, the Pods from the `gallery-dep` Deployment will be evicted from the `desktop-worker2` node because they no longer have a matching Toleration for the new Taint. You can see this in `k9s` as the Pods have been terminated and are stuck in a pending state again. Let's delete the Deployment, change the Taint again, and deploy another manifest without any Node Selectors and Tolerations:

```bash
kubectl delete deployment gallery-dep
kubectl taint nodes desktop-worker2 workloads=frontend:NoExecute-
kubectl taint nodes desktop-worker2 workloads=frontend:PreferNoSchedule
kubectl apply -f deployment.yaml
```

Take a look at the Pods in `k9s`. You will see that all Pods are scheduled on the `desktop-worker` node because the Taint has a `PreferNoSchedule` effect and the Pods do not have a matching Toleration. However, if that node is unavailable, Pods can be rescheduled on the `desktop-worker2` node because the Taint is not a strict requirement. Let's simulate a node maintenance and drain the `desktop-worker` node. You can do this using the following command:

```bash
kubectl drain desktop-worker --ignore-daemonsets --delete-emptydir-data
```

You will see in `k9s` that all Pods on the `desktop-worker` node are evicted and rescheduled on the `desktop-worker2` node. In addition, new Pods are not scheduled on the `desktop-worker` node because it is in a `SchedulingDisabled` state (see the nodes in `k9s`). Try scaling the Deployment again (e.g., `kubectl scale deployment gallery-dep --replicas=5`) and you will see that the new Pods are only scheduled on `desktop-worker2` for now. Let's make the `desktop-worker` node schedulable again by uncordoning it and delete the Deployment and Taint for now. You can do this using the following commands:
...
You should see in `k9s` that all 6 Pods from the `gallery-dep` Deployment are distributed across all nodes in the cluster. Now, let's simulate a node maintenance and drain the `desktop-worker` node. You can do this using the following command:
...
Clean up everything by deleting the PDB and Deployment, and uncordon the `desktop-worker` node:
