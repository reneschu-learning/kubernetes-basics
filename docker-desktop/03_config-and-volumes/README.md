...
Then open a browser and navigate to `http://localhost:8080`. This version of the application has a tab named `Config`, which displays the values of several variables and a secret. For now, only the variable `CONFIG_VAR1` shows a value.
...
Then open a browser and navigate to `http://localhost:8080` and switch to the `Config` tab. Now, both the variable `CONFIG_VAR1` and the variable `CONFIG_FILE` show values. Currently, the `CONFIG_FILE` variable points to a configuration file that does not exist, so you will see an error message in the field `CONFIG_FILE content`. We will fix this later.

What happens if we change the value of `CONFIG_VAR1` in the ConfigMap? Let's try it out. Edit the ConfigMap and change the value of `CONFIG_VAR1` to `New value from ConfigMap`. Then, apply the changes using the following command:
...
For now, let's delete everything we have created so far using the following commands:
